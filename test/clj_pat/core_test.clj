...
(deftest test-request-error
  (let [fake-body (json/generate-string {:code 404 :error "Postcode is not valid"})]
    (with-fake-routes {"http://example.com" (fn [request] {:status 404 :headers {} :body fake-body})}
      (is (= {:error {:status 404 :message "Postcode is not valid"} } (get-data "http://example.com"))))))
